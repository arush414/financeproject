...
def blackScholes_model(S, K, sd, r, T):
    B = K*np.exp(-r*T)
    lg = np.log(S/B)
...
def n_step(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            n = int(form.data['number_of_steps'])
            S = float(form.data['stock_price'])
            K = float(form.data['spot_price'])
            r = float(form.data['risk_free_interest_rate'])
            T = float(form.data['maturity_time_in_months'])
            u = float(form.data['upstock_rate'])
            d = float(form.data['downstock_rate'])
            T = T/12
            r = r/100
            d = 1-d/100
            u = 1+u/100
...
def black_scholes(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            r = r/100
            sd = sd/100
            call = blackScholes_model(S, K, sd, r, T)
            put = parity(call, K, r, T, S)
            context['call'] = call
            context['put'] = put
            return render(request, 'blacksholes.html', context)
    else:
